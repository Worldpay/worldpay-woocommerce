...
class WorldPay_PaymentForm
{
    ...
    public static function existing_details_fields(WorldPay_CardDetails $cardDetails)
    {
        ?>
            <p class="form-row form-row-first">
                ...
                <input id="worldpay_saved_card" type="text" class="input-text" disabled autocomplete="off" value="<?= $cardDetails->maskedCardNumber ?>"/>
                <input id="worldpay_token" type="hidden" data-worldpay="token" value="<?= $cardDetails->token ?>"/>
            </p>
        ...
        ?>
            <select id="worldpay_expiration_month" data-worldpay="exp-month">
                ...
                <?php for($i = 1; $i <=12; $i++) { ?>
                    ...
                    <option value="<?= $formatted ?>"><?= $formatted ?> - <?= date("F", mktime(0, 0, 0, $i, 10)) ?></option>
                <?php } ?>
            </select>
        <?php
    }
